...
- 0.8 adds one precondition on top of this ADR's model slot, refusing `unload_model` while a chat run holds a lease on the loaded model; that refusal and the run-lease/transport decisions it depends on are recorded separately in ADR 0012.
