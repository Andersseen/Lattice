...
Engineering plan, 2026-09-06. Baseline: unpublished `0.1.0` foundation at `2e8eb7e`. Status updated 2026-09-16: `0.2` is archived, the `0.3` desktop security baseline is implemented on the active branch, `0.4` persistent settings is implemented with native GUI restart evidence pending before archive, `0.5` ModelRuntime discovery is implemented with real llmster smoke evidence pending before archive, `0.6` llmster lifecycle is implemented on the active branch with a real successful start/stop cycle and the P1 profile pending before archive, `0.7` installed model management is implemented on the active branch with qualified-candidate confirmation and the P2 profile pending before archive, `0.8` local streaming chat is implemented on the active branch (provider port, local OpenAI-compatible adapter, Tauri streaming channel, browser-simulated and native Chat UI, full automated test/build evidence) with a real llmster SSE sample and the completed P2 profile pending before archive, `0.9` conversation persistence is implemented on the active branch (`ConversationStore` sibling SQLite domain, create/list/reopen/delete IPC, bounded checkpointing, restart-to-`interrupted` reconciliation, History UI, full automated test/build evidence) with a real native GUI restart and a real llmster-backed session pending before archive, and `0.10` OS-secure credentials is implemented on the active branch (`CredentialStore` sibling SQLite domain storing only reference metadata, `security-framework`-backed macOS Keychain storage with genuine — not faked — automated round-trip tests, `osascript`-based native secure entry, list/create/replace/delete IPC with no secret-returning command, Settings UI, full automated test/build evidence) with a real interactive native secure-entry dialog pending before archive, and `0.11` remote OpenAI-compatible chat is implemented on the active branch (`ProviderProfileStore` sibling SQLite domain with destination-bound credential binding and consent, one `openai_compatible` adapter for local and remote destinations over rustls, per-request chat target, Settings/Chat UI, full automated test/build evidence plus a real unauthenticated HTTPS normalization smoke) with an authenticated remote streaming smoke and native GUI use pending before archive (this machine's llmster installation cannot wake without a one-time GUI first run, and this sandbox has no interactive WindowServer/Aqua session for the native dialog either; see `docs/verification/0.6-llmster-lifecycle.md`, `docs/verification/0.7-installed-model-management.md`, `docs/verification/0.8-local-streaming-chat.md`, `docs/verification/0.9-conversation-persistence.md`, `docs/verification/0.10-os-secure-credentials.md` and `docs/verification/0.11-remote-openai-compatible-chat.md`). Future release entries remain planned until their OpenSpec changes are accepted and verified. No dates, tags or published releases are created by this plan.
...
| 0.10    | Done, archive/evidence | Native secure credential provisioning.                      |
| 0.11    | Done, archive/evidence | Remote OpenAI-compatible chat.                              |
...
**Status:** implementation done on the active branch. The `providers` module (canonical port, local OpenAI-compatible adapter, reader/orchestrator cancellation), the Tauri streaming channel/commands, and a browser-simulated plus native Chat UI are implemented with disposable-fixture Rust tests, Vitest/Playwright coverage, and a passing native (`--no-bundle`) release build; see `docs/verification/0.8-local-streaming-chat.md`. A real llmster SSE sample and the P2 active-stream profile remain pending before archive, blocked on the same headless-daemon-wake gap 0.6/0.7 already recorded.
...
- [x] Incremental text and cancel work with exactly one terminal outcome per request.
- [x] Stale events cannot reach a new run; crash/error frees the active slot.
- [ ] Named chat profiles pass smoke; context/output/queue limits and P2 evidence are verified. (Pending: real llmster SSE sample and P2 profile; see verification doc.)
...
**Status:** implementation done on the active branch. `CredentialStore` (a third sibling SQLite domain, schema version 6) stores only reference metadata; the secret lives exclusively behind a `SecretStore`/`SecretPrompt` trait pair with a real macOS Keychain (`security-framework`) implementation and an `osascript`-based native entry prompt, both covered by genuine — not only faked — automated tests for the Keychain side. No IPC command ever returns a secret. Disposable-fixture and real-keychain Rust tests, OpenSpec/contract/TypeScript/web-build/E2E/native-build checks all pass; see `docs/verification/0.10-os-secure-credentials.md`. A real interactive native secure-entry dialog remains pending before archive, blocked on this sandbox having no interactive WindowServer/Aqua session (the same class of environment gap 0.6–0.9 already recorded for llmster).
...
- [x] Native entry/replace/delete yield accurate reference status after restart.
- [x] No secret leaks; locked/unavailable storage never falls back to plaintext.
- [x] Tested native secure-input dependency/implementation and platform limits are documented.
...
**Status:** implementation done on the active branch as OpenSpec change `remote-openai-compatible-chat`. `ProviderProfileStore` (a fourth sibling SQLite domain, schema version 7) stores HTTPS-only profiles whose credential binding and consent are bound to one endpoint and cleared when it changes; `StartChatStreamRequest` gains a per-request `target`, resolved in Rust (model, consent, then `resolve_secret`) before anything is sent or persisted; `local_openai.rs` became one `openai_compatible` adapter serving local and remote destinations with rustls, no redirects, no environment proxy, bounded stream lines, normalized `provider.*` errors and best-effort connection close on cancel (ADR 0014). Rust, Vitest, Playwright, OpenSpec, web and native `--no-bundle` checks pass, and an opt-in unauthenticated HTTPS request to OpenAI normalizes to `provider.auth_failed`. An authenticated remote streaming smoke (no maintainer credential here) and native GUI use remain before archive; see `docs/verification/0.11-remote-openai-compatible-chat.md` and `docs/remote-providers.md`.
...
- [x] Local → remote → local retains history and sends only on explicit request/consent.
- [x] Adapter details stay outside domain/UI; failures/cancel follow shared outcomes.
- [ ] Tested endpoint/protocol subset is documented without universal compatibility claims. (Subset and conformance matrix documented in `docs/remote-providers.md`; pending: an authenticated streaming smoke against a named endpoint/model, see verification doc.)
...
**Only 0.12 — Anthropic adapter.** 0.2 is archived; 0.3 through 0.11 are implemented on the active branch with their recorded evidence limits (see `docs/verification/`). Start from the accepted `remote-openai-compatible-chat` change and ADR 0014: `ChatTarget`/`StartChatStreamRequest.target` is the per-request selection, `profiles::prepare_remote_target` is the remote authorization use case (model, destination-bound consent, then `resolve_secret`), and `providers::openai_compatible` is the one OpenAI-compatible protocol adapter. 0.12 is the first genuinely different protocol, so it is where a provider boundary wider than `CompletionTarget` is earned; decide in its design whether profiles gain a protocol discriminator or Anthropic gets its own profile shape, without breaking 0.11's stored profiles or consent semantics. Read AGENTS, current architecture/specs, this scope and linked v1 constraints; explore current official Anthropic Messages API streaming docs and pin the tested API version; then create one OpenSpec change for `providers` (Anthropic deltas) before coding. Do not implement Gemini, tools, or any additional API surface as part of `0.12`. Like 0.11, a full verification needs genuine network access and a real Anthropic credential — flag that dependency explicitly rather than fabricating authenticated smoke evidence.
