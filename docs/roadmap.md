...
**Only 0.11 — Remote OpenAI-compatible chat.** 0.2 is archived; 0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, and 0.10 are implemented on the active branch with their recorded evidence limits (see `docs/verification/`). Start from the accepted `os-secure-credentials` change and ADR 0013 (`CredentialStore`/`resolve_secret` is the only path to a usable secret; 0.11 is its first real Rust-internal consumer). Read AGENTS, current architecture/specs, this scope and linked v1 constraints; explore actual prerequisite state (the canonical `ChatRequest`/`ChatStreamEvent` port 0.8 already defines and that 0.11 must satisfy without redesign, 0.9's conversation persistence this change must remain compatible with, the pinned compatible-protocol subset to target); then create one OpenSpec change for `providers` (remote deltas) before coding. Do not implement Anthropic/Gemini adapters, agent tools, or any additional API surface as part of `0.11`. This minor requires genuine external network access and a real remote credential to fully verify — flag that dependency explicitly rather than fabricating remote-smoke evidence.
