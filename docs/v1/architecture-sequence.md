...
Current implementation status as of 2026-09-16: the 0.2 application API boundary is archived; 0.3 desktop security, 0.4 settings/local storage, 0.5 ModelRuntime discovery, 0.6 llmster lifecycle (owned/attached start-stop), 0.7 installed model management, 0.8 local streaming chat (the `Provider` boundary's first consumer), 0.9 conversation persistence (a sibling `ConversationStore` boundary), and 0.10 OS-secure credentials (a third sibling `CredentialStore` boundary storing only reference metadata, with the secret itself isolated behind macOS Keychain/native-entry adapters and never reaching Rust's own SQLite, IPC, or Angular), and 0.11 remote OpenAI-compatible chat (a fourth sibling `ProviderProfileStore`, a per-request local/remote chat target, and one OpenAI-compatible adapter proving provider substitution without changing the 0.8 port; ADR 0014) are implemented on the active roadmap branch with their recorded evidence limits. The next product boundary to add is 0.12 Anthropic, the first genuinely different provider protocol.
