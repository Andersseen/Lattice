...
Current implementation status as of 2026-09-13: the 0.2 application API boundary is archived; 0.3 desktop security, 0.4 settings/local storage, 0.5 ModelRuntime discovery, 0.6 llmster lifecycle (owned/attached start-stop), 0.7 installed model management, 0.8 local streaming chat (the `Provider` boundary's first consumer), and 0.9 conversation persistence (a sibling `ConversationStore` boundary, not a `Provider` or `SettingsStore` extension) are implemented on the active roadmap branch with their recorded evidence limits. The next product boundary to add is 0.10 OS-secure credentials.
